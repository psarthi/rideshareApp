...
import com.parift.rideshare.adapter.RewardReimbursementTransactionListAdapter;
 ...
 * {@link RewardReimbursementTransactionListFragment.OnFragmentInteractionListener} interface
 * to handle interaction events.
 * Use the {@link RewardReimbursementTransactionListFragment#newInstance} factory method to
 * create an instance of this fragment.
 */
public class RewardReimbursementTransactionListFragment extends BaseFragment {
    public static final String TAG = RewardReimbursementTransactionListFragment.class.getName();
    ...
    private List<RewardReimbursementTransaction> mRewardReimbursementTransactions = new ArrayList<>();
    private BasicUser mUser;
    private String GET_REWARD_REIMBURSEMENT_TRANSACTIONS_URL;


    public RewardReimbursementTransactionListFragment() {
    ...
    public static RewardReimbursementTransactionListFragment newInstance() {
        Logger.debug(TAG, "newInstance Called");
        RewardReimbursementTransactionListFragment fragment = new RewardReimbursementTransactionListFragment();
        return fragment;
    }
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        GET_REWARD_REIMBURSEMENT_TRANSACTIONS_URL = APIUrl.GET_REWARD_REIMBURSEMENT_TRANSACTIONS.replace(APIUrl.USER_ID_KEY,Long.toString(mUser.getId()));
    }
    ...
    private void loadInitialData() {
        ...
        String URL = GET_REWARD_REIMBURSEMENT_TRANSACTIONS_URL.replace(APIUrl.PAGE_KEY, Integer.toString(0));
        ...
        RESTClient.get(URL, null, new RSJsonHttpResponseHandler(mCommonUtil){
            ...
            public void onSuccess(int statusCode, Header[] headers, JSONArray response) {
                if (isAdded()) {
                    ...
                    Type listType = new TypeToken<ArrayList<RewardReimbursementTransaction>>() {
                    }.getType();
                    mRewardReimbursementTransactions = new Gson().fromJson(response.toString(), listType);
                    Logger.debug(TAG, "Size of initial set of data is: " + mRewardReimbursementTransactions.size());
    ...
    private void setAdapter() {
        ...
        mAdapter = new RewardReimbursementTransactionListAdapter(mRewardReimbursementTransactions, this);
        mRecyclerView.setAdapter(mAdapter);
        if (mRewardReimbursementTransactions.size()==0) {
    ...
    public void loadNextDataFromApi(final int offset) {
        ...
        String URL = GET_REWARD_REIMBURSEMENT_TRANSACTIONS_URL.replace(APIUrl.PAGE_KEY, Integer.toString(offset));
        ...
        RESTClient.get(URL, null, new RSJsonHttpResponseHandler(mCommonUtil){
            ...
            public void onSuccess(int statusCode, Header[] headers, JSONArray response) {
                if (isAdded()) {
                    ...
                    List<RewardReimbursementTransaction> newRewardReimbursementTransactions = new Gson().fromJson(response.toString(), listType);
                    //Since object is pass by reference, so when you drawable.add in mRides, this will be reflected everywhere
                    mRewardReimbursementTransactions.addAll(newRewardReimbursementTransactions);
                    Logger.debug(TAG, "Size of new set of data is: " + newRewardReimbursementTransactions.size() + " :Updated count is:" + mRewardReimbursementTransactions.size());
                    mAdapter.notifyItemRangeInserted(mAdapter.getItemCount(), mRewardReimbursementTransactions.size() - 1);
                }
            }
        });
    }
    ...
    public interface OnFragmentInteractionListener {
        ...
        void onRewardReimbursementTransactionListFragmentInteraction(String data);
    }
}
